...
- `DATE` and `DATETIME` cells hold the **same** value — an ISO-8601 UTC instant from `toISOString()`. They differ only in the web editor (`DATETIME` adds a `TimePicker` beside the `Calendar`) and the display format.
- Cell **type validation** lives in `tableCellValidation.validate()` (`core/shared/.../tables/cell-validation.ts`) and is shared verbatim by the web editor (blocks the cell at edit time) and `POST /v1/records/batch` (per-cell `VALIDATION` errors). The single-record create/update endpoints deliberately stay lenient — flows and the Tables piece can still write arbitrary text into any column, so don't "fix" them to validate.
...
- The web editor **stages cell edits locally** (zustand `cellStates`: `dirty`/`saving`/`saved`/`error`/`conflict` keyed `{recordUuid}:{fieldUuid}`) and saves them in one `POST /v1/records/batch` call per Save click; the endpoint returns a per-record result array aligned to the request order, so one bad record never rejects the batch. New-record creation and field ops still POST immediately.
- **Optimistic concurrency**: each dirty cell is sent with `baseUpdated` = the cell's `updated` token the client last saw. The batch endpoint compares it (instant comparison, `FOR UPDATE` on the record row) and returns `CONFLICT` + the current server record instead of overwriting. Cells without a server row send no token. Both upsert paths (batch and single update) bump `cell.updated` on every write — TypeORM `upsert` sets `updateDate` columns to `DEFAULT` (CURRENT_TIMESTAMP) unless an explicit value is provided.
