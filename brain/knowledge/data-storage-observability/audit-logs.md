...
- One-time download: `POST /exports/:id/download-link` mints a 15-min JWT (audience `AUDIT_LOG_EXPORT_DOWNLOAD`, per-token `jti`) for an absolute URL; the public `GET /v1/audit-events/exports/:id/download?token=` route is mounted **outside** the `auditLogEnabled` feature-gate (the JWT is the credential) and atomically marks the `jti` used in Redis (`SET NX EX`) — the second use 400s before touching the file.
