...
- **Project Trigger Calendar** (`GET /v1/trigger-calendar/calendar`, `trigger/trigger-calendar/`): reads non-deleted, non-simulate `trigger_source` rows for **ENABLED** flows only (flow.status = ENABLED, not DELETING, joined to `publishedVersionId`) and computes upcoming occurrences **server-side only** with `cron-parser` 4.9 — the exact library/version BullMQ uses for `repeat.pattern`, so the browser never parses cron. The route requires project membership but **not** READ_FLOW (and the React route/tab carry no `RoutePermissionGuard`/`checkAccess(READ_FLOW)` for the same reason): members without READ_FLOW must reach the page, see an amber "N flow(s) are hidden" banner plus an empty-state message, and get `restrictedCount` with all other lists empty — a READ_FLOW gate (backend *or* frontend) would block them before that. Restricted count is computed against the *filtered* candidate rows (flowIds/folderIds honoured); only rows are hidden, never their existence count. CRON sources become calendar occurrences (with per-occurrence UTC offset and a DST flag, marked only when the offset differs from the *previous* occurrence — never on the first occurrence); `INTERVAL` sources (`every_x_minutes`, default poll interval) cannot be placed on a wall-clock calendar and are listed separately alongside WEBHOOK/APP_WEBHOOK/MANUAL. Invalid cron / unknown timezone / invalid interval are returned in `issues`, never dropped; `conflicts` groups occurrences of *different* flows landing in the same minute (server-only; hidden flows can never enter it). Filters: flowIds, folderIds (the `NULL` sentinel means uncategorized), instance timezones, window 1–90 days (default 7). The web grid (`buildDayBuckets` in `lib/day-buckets.ts`) pre-creates a bucket for **every local day** the window touches (in the display timezone), including trigger-free days, iterating local calendar dates with offsets from `Intl.DateTimeFormat` — do **not** use dayjs `.utc().tz(z).startOf('day')` for this: it keeps the pre-conversion wall clock while swapping the offset, so when the browser TZ differs from the display TZ the grid starts a day early and gains an extra bucket (only `.format()` on an instant is TZ-safe). Occurrences whose flow is absent from the visible `scheduled` map are also dropped client-side, so disabled/invisible flows cannot render or collide. `UTC` is a valid schedule timezone but is **not** in `Intl.supportedValuesOf('timeZone')` — validate timezones with a `DateTimeFormat` probe, not the IANA list.
