...
describe('agentWorkerTools', () => {
    describe('isSuccessResult', () => {
        it('returns true for MCP success result', () => {
            expect(agentWorkerTools.isSuccessResult(mcpSuccess('Done'))).toBe(true)
        })

        it('returns false for MCP failure result', () => {
            expect(agentWorkerTools.isSuccessResult(mcpFailure('Something broke'))).toBe(false)
        })

        it('returns false for timeout result', () => {
            const result = { content: [{ type: 'text', text: '⏳ Action still running after 120s.' }] }
            expect(agentWorkerTools.isSuccessResult(result)).toBe(false)
        })

        it('returns false for structured error result', () => {
            expect(agentWorkerTools.isSuccessResult({ success: false, error: 'No projects' })).toBe(false)
        })

        it('returns false for isError flag', () => {
            expect(agentWorkerTools.isSuccessResult({ content: [{ type: 'text', text: 'error' }], isError: true })).toBe(false)
        })

        it('returns false for null', () => {
            expect(agentWorkerTools.isSuccessResult(null)).toBe(false)
        })

        it('returns false for string', () => {
            expect(agentWorkerTools.isSuccessResult('some string')).toBe(false)
        })

        it('returns false for object without content array', () => {
            expect(agentWorkerTools.isSuccessResult({ data: 'something' })).toBe(false)
        })
    })
    ...
    describe('extractResultText', () => {
        it('extracts text from MCP content array', () => {
            ...
            expect(agentWorkerTools.extractResultText(result)).toBe('Hello\nWorld')
        })

        it('returns error field directly', () => {
            expect(agentWorkerTools.extractResultText({ success: false, error: 'No projects available' })).toBe('No projects available')
        })

        it('returns raw string as-is', () => {
            expect(agentWorkerTools.extractResultText('raw text')).toBe('raw text')
        })

        it('JSON-stringifies non-object non-string values', () => {
            expect(agentWorkerTools.extractResultText(42)).toBe('42')
            expect(agentWorkerTools.extractResultText(null)).toBe('null')
        })

        it('JSON-stringifies objects without content or error', () => {
            expect(agentWorkerTools.extractResultText({ data: 'value' })).toBe('{"data":"value"}')
        })
    ...
    describe('extractUserFacingError', () => {
        it('prefers the clean summary from _meta', () => {
            ...
            expect(agentWorkerTools.extractUserFacingError({ result, meta })).toBe('The request body contains invalid JSON. (400)')
        })
        ...
        it('strips emoji and retry coaching when no summary is present', () => {
            ...
            expect(agentWorkerTools.extractUserFacingError({ result })).toBe('Action failed (run abc): Something broke.')
        })

        it('truncates very long raw errors', () => {
            const longError = 'x'.repeat(500)
            const out = agentWorkerTools.extractUserFacingError({ result: { error: longError } })
    ...
    describe('ap_execute_action batch mode', () => {
        it('calls executeTool for each item and emits progress events', async () => {
            ...
            const tools = agentWorkerTools.createCrossProjectTools({ executeTool, eventEmitter, waitForApproval: vi.fn().mockResolvedValue({ outcome: 'approved' }), guides: {}, taintState: { tainted: false } })
    ...
    describe('ap_execute_action taint gate', () => {
        it('forces the approval gate for a non-read-only action once the turn is tainted', async () => {
            ...
            const tools = agentWorkerTools.createCrossProjectTools({ executeTool, eventEmitter, waitForApproval, guides: {}, taintState: { tainted: true } })
        ...
        it('does not gate the same action when the turn is untainted', async () => {
            ...
            const executeTool = vi.fn().mockResolvedValue(mcpSuccess('ok'))
            const waitForApproval = vi.fn().mockResolvedValue({ outcome: 'approved' })
            const tools = agentWorkerTools.createCrossProjectTools({ executeTool, eventEmitter, waitForApproval, guides: {}, taintState: { tainted: false } })
    ...
    describe('truncateLargeResult', () => {
        ...
        it('truncates based on byte size, not UTF-16 length (multibyte)', () => {
            ...
            const result = agentWorkerTools.truncateLargeResult(emojiHeavy)
    ...
    describe('truncateLargeResult', () => {
        ...
        it('caps the exact incident shape — MCP { content: [{ text: <huge> }] }', () => {
            ...
            const out = agentWorkerTools.truncateLargeResult(input)
        ...
        it('caps a huge primitive string', () => {
            const out = agentWorkerTools.truncateLargeResult('w'.repeat(1200 * 1024))
            expect(serializedBytes(out)).toBeLessThanOrEqual(MAX_RESULT_SIZE_BYTES)
        })
        ...
        it('does not throw on a circular object and still caps the result', () => {
            ...
            expect(() => agentWorkerTools.truncateLargeResult(circular)).not.toThrow()
            expect(serializedBytes(agentWorkerTools.truncateLargeResult(circular))).toBeLessThanOrEqual(MAX_RESULT_SIZE_BYTES)
        })

        it('returns small results unchanged', () => {
            const input = { content: [{ type: 'text', text: '✅ Listed 3 connections' }] }
            expect(agentWorkerTools.truncateLargeResult(input)).toBe(input)
        })
    })
    ...
    describe('createEmailTools / ap_send_email', () => {
        ...
        function setup({ sendImpl, approved = true }: { sendImpl?: () => Promise<SendAgentEmailResponse>, approved?: boolean } = {}) {
            ...
            const tools = agentWorkerTools.createEmailTools({ sendEmail, eventEmitter, userEmail: SELF_EMAIL, waitForApproval })
