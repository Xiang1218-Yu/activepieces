import { agentToolClassification, PersistedAgentPartType, PersistedAgentRole, PersistedToolCallStatus } from '@activepieces/shared'
...
function collectToolCalls(uiMessages: unknown[]): ToolCall[] {
    ...
    for (const message of uiMessages) {
        ...
        for (const part of partsOf(message)) {
            if (asString(part['type']) !== PersistedAgentPartType.TOOL_CALL) continue
...
function hasSuccessfulReceipt(uiMessages: unknown[]): boolean {
    for (const message of uiMessages) {
        if (!isAssistant(message)) continue
        for (const part of partsOf(message)) {
            if (asString(part['type']) === PersistedAgentPartType.ACTION_RECEIPT && part['status'] === 'success') {
                return true
            }
        }
    }
...
function isSuccess(call: ToolCall): boolean {
    ...
    return call.status === PersistedToolCallStatus.COMPLETED
        && !agentToolClassification.hasFailureTextPrefix(call.outputText)
        && !call.outputText.includes(BREAKER_MARKER)
}

function isFailure(call: ToolCall): boolean {
    return call.status === PersistedToolCallStatus.ERROR || agentToolClassification.hasFailureTextPrefix(call.outputText)
}
...
function lastAssistantText(uiMessages: unknown[]): string {
    ...
    for (const message of uiMessages) {
        if (!isAssistant(message)) continue
        for (const part of partsOf(message)) {
            if (asString(part['type']) === PersistedAgentPartType.TEXT) {
...
function isAssistant(message: unknown): message is Record<string, unknown> {
    return isRecord(message) && message['role'] === PersistedAgentRole.ASSISTANT
}
