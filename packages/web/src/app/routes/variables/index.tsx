...
import { VariableValueCell } from '@/app/variables/variable-value-cell';
import {
  BulkAction,
  DataTable,
  DataTableFilters,
...
function VariablesPage() {
  ...
  const [editing, setEditing] = useState<VariableListItem | undefined>(
    undefined,
  );
  const [deleting, setDeleting] = useState<VariableListItem | undefined>(
    undefined,
  );
  const [selectedRows, setSelectedRows] = useState<VariableListItem[]>([]);
  ...
  const request: ListVariablesRequestQuery = {
    projectId,
    cursor,
    limit,
    name,
    ...
    updatedAfter,
    updatedBefore,
    usedInFlows: usedInFlows.length > 0 ? usedInFlows : undefined,
    includeValues: 'true',
  };
  ...
  const {
    ...
    isError,
    refetch,
  } = variablesQueries.useVariables({ request });
  ...
  const filters: DataTableFilters<keyof VariableListItem>[] =
    ownerColumnHooks.useOwnerColumnFilter<VariableListItem>(
      [
        {
          type: 'input',
          title: t('Name'),
        ...
        },
        {
          ...
          icon: Shapes,
          options: [
            { label: t('Sensitive'), value: VariableType.SECRET, icon: Lock },
            {
              label: t('Plain text'),
        ...
        },
        {
          type: 'select',
          title: t('Usage'),
          accessorKey: 'usedInFlows',
          icon: Workflow,
          options: [
            { label: t('Used'), value: 'true' },
            { label: t('Unused'), value: 'false' },
          ],
        },
        {
          type: 'date',
  ...
  const columns: ColumnDef<RowDataWithActions<VariableListItem>, unknown>[] =
    ownerColumnHooks.useOwnerColumn<VariableListItem>(
      [
        {
          accessorKey: 'name',
          size: 240,
        ...
        },
        {
          ...
          cell: ({ row }) => <VariableValueCell variable={row.original} />,
        },
        {
          accessorKey: 'type',
          size: 130,
          ...
          cell: ({ row }) =>
            row.original.type === VariableType.SECRET ? (
        ...
        },
        {
          ...
          cell: ({ row }) =>
            row.original.usedInFlows ? (
              <Badge variant="success">{t('Used')}</Badge>
            ) : (
              <Badge variant="accent">{t('Unused')}</Badge>
            ),
        },
        {
          accessorKey: 'updated',
  ...
  const bulkActions: BulkAction<VariableListItem>[] = useMemo(
    () => [
      {
